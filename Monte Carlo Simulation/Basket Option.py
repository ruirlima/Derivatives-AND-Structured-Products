...
K = 0                                       # 0 for ATM option, otherwise insert strike
...
# Update strike price in case assumption is ATM option
if K == 0:
    K = np.sum(np.multiply(np.array(df_S.loc[1,(1,)]),weight))

# Compute index value at maturity
final_index = [np.sum(np.multiply(np.array(df_S.loc[days,(sim,)]),weight)) for sim in range(1,simulations+1)]
# Option prices
